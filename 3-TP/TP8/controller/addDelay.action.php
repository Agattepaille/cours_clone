...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Vérifier si un retard a été sélectionné
    if (!empty($delay)) {
        // Appeler la fonction du modèle pour ajouter le retard
        DBManagement::addDelay($_POST['user_ID'], $delay);
        ...
        */
        ...
        $recipients = [
        [
            //récupérer le mail de la personne dénoncée
            'Email' => $mailRecipient
        ]
        ];
        ...
        */
        ...
        // Rediriger ou effectuer d'autres actions après l'ajout du retard
        header('Location: ../view/index.php');
        exit();
    } else {
        // Option par défaut sélectionnée, aucune action nécessaire
        header('Location: ../view/index.php?error=no-delay-selected');
        exit();
    }
} else {
    // Gestion d'une tentative d'accès direct au script sans POST
